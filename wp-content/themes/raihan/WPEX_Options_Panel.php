...
class WPEX_Options_Panel {
    ...
    public function render_image_field( $args ) {
        ...
        <div class="image-div">
            <input
                type="text" class="image_field"
                name="<?php echo $this->option_name; ?>[<?php echo esc_attr( $args['label_for'] ); ?>]"
                value="<?php echo esc_attr( $value ); ?>">
                <input type="button" class="button button-secondary mytheme_image_button" value="Upload Image"/><br>
                <img width="100px" src="<?php echo esc_attr( $value ); ?>" alt="">
        </div>
            <?php if ( $description ) { ?>
